...
namespace HMS_UI.HelperFunctions
{
    public static class TransactionHelpers
    {
        ...
        public static bool ValidateTransaction(ref DataGridView patientData,
            ref DateTimePicker TransactDateTimePicker, ref ComboBox statusComboBox, string AmountText, string PaidText)
        {
            ...
            if (!TransactDateTimePicker.Checked)
            {
                MessageBox.Show("Date and Time must be selected.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return false;
            }

            if (!Enum.IsDefined(typeof(TransactionStatus), statusComboBox.SelectedItem))
            {
                MessageBox.Show("Invalid Status Selected'.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return false;
            }

            if (string.IsNullOrEmpty(AmountText))
            {
                MessageBox.Show("AmountText cannot be empty or null.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return false;
            }

            if (string.IsNullOrEmpty(PaidText))
            {
                MessageBox.Show("PaidText cannot be empty or null.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return false;
            }

            if (!decimal.TryParse(AmountText, out decimal amount))
            {
                MessageBox.Show("AmountText is not a valid decimal.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return false;
            }

            if (!decimal.TryParse(PaidText, out decimal paid))
            {
                MessageBox.Show("PaidText is not a valid decimal.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return false;
            }
